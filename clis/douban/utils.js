 ...
 */
import { ArgumentError, CliError, EmptyResultError } from '@yangshengzhou/leafcli/errors';
...
async function ensureDoubanReady(page) {
  ...
  `);
    if (state?.blocked) {
        throw new CliError('AUTH_REQUIRED', 'Douban requires a logged-in browser session before these commands can load data.', 'Please sign in to douban.com in the browser that leafcli reuses, then rerun the command.');
    }
}
