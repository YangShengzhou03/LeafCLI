 ...
 * Cross-platform: works on Linux/macOS/Windows via leafcli's CDP browser automation.
 */

import { htmlToMarkdown } from '@yangshengzhou/leafcli/utils';
import { ArgumentError, AuthRequiredError, CommandExecutionError, TimeoutError } from '@yangshengzhou/leafcli/errors';
...
function debugChatGPTModel(message) {
    if (process?.env?.leafcli_CHATGPT_MODEL_DEBUG) {
...
function buildComposerLocatorScript() {
    const markerAttr = 'data-leafcli-chatgpt-composer';
    return `
      const isVisible = (el) => {
...
export function requireNonEmptyPrompt(prompt, commandName) {
    ...
    if (!text) {
        throw new ArgumentError(
            ...
            `Example: leafcli ${commandName} "hello"`,
        );
    }
    return text;
}
...
export function parseChatGPTConversationId(value) {
    ...
    if (/^https?:\/\//i.test(raw)) {
        ...
        throw new ArgumentError(
            ...
            'Example: leafcli chatgpt detail https://chatgpt.com/c/123e4567-e89b-12d3-a456-426614174000',
        );
    }
    ...
    throw new ArgumentError(
        'chatgpt detail requires a conversation id or chatgpt.com /c/<id> URL',
        'Example: leafcli chatgpt detail 123e4567-e89b-12d3-a456-426614174000',
    );
}
...
export function parseChatGPTProjectId(value) {
    ...
    if (/^https?:\/\//i.test(raw) || raw.startsWith('/')) {
        ...
        throw new ArgumentError(
            ...
            'Example: leafcli chatgpt project-file-add report.pdf --id 12345678',
        );
    }
