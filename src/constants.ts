...
export function unsupportedDaemonPortEnvMessage(value?: string): string {
  ...
  return `leafcli_DAEMON_PORT is no longer supported${suffix}. ` +
    `The leafcli Chrome extension can only connect to localhost:${DEFAULT_DAEMON_PORT}. ` +
    'Unset leafcli_DAEMON_PORT and rerun leafcli.';
}

/**
 * True when leafcli_DAEMON_PORT carries no real configuration: unset, empty,
 * or equal to the default port. Launchers (notably leafcliApp) inject the
