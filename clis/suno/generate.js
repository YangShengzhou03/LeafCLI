/**
 * `leafcli suno generate` — submit a Suno music-generation request, wait for
    ...
    func: async (page, kwargs) => {
        ...
        if (!isCustom && !description.trim()) {
            throw new ArgumentError(
                ...
                'Examples:\n  leafcli suno generate "lo-fi study beat, 80 bpm"\n  leafcli suno generate --lyrics "[Verse]\\n..." --tags "synthwave, 120 bpm"',
            );
        }
        ...
        for (const clip of clips) {
            ...
            const writtenSummary = result.written
                .map(w => w.ok ? `${w.format}:${displayPath(w.file)}` : `${w.format}:[失败](${w.reason})`)
                .join(' | ');
            ...
            rows.push({
                status: anyFailed ? '[警告] partial' : '[成功] saved',
                clip: clip.id.slice(0, 8),
                title: clip.title || '(untitled)',
                files: `[文件] ${writtenSummary}${skippedSummary}`,
                link: `[链接] ${link}`,
            });
        }
        return rows;
