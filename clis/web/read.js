 ...
 *
 * Usage:
 *   leafcli web read --url "https://www.anthropic.com/research/..." --output ./articles
 *   leafcli web read --url "https://..." --download-images false
 */
import { cli, Strategy } from '@yangshengzhou/leafcli/registry';
import { downloadArticle } from '@yangshengzhou/leafcli/download/article-download';
      ...
      (() => {
        ...
        const collectEmptyContainers = (root, scope, baseUrl) => {
          ...
          root.querySelectorAll?.(likely).forEach((el) => {
            if (scope === 'main' && el.closest?.('[data-leafcli-iframe-source]')) return;
        ...
        const buildFrameSection = (frameBody, desc, fallbackLabel) => {
          ...
          section.setAttribute('data-leafcli-iframe-source', desc.src);
...
const command = cli({
    ...
    func: async (page, kwargs, debug = false) => {
        ...
        const shouldDiagnose = boolish(kwargs.diagnose) || debug || !!process.env.leafcli_VERBOSE;
