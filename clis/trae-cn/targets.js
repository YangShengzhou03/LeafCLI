...
function normalizeEndpoint(value) {
  const endpoint = String(value || process.env.leafcli_CDP_ENDPOINT || 'http://127.0.0.1:39240').trim();
  if (!endpoint) {
    throw new ArgumentError('Set leafcli_CDP_ENDPOINT, for example http://127.0.0.1:39240');
  }
...
export const targetsCommand = cli({
  ...
  example: 'leafcli_CDP_ENDPOINT=http://127.0.0.1:39240 leafcli trae-cn targets -f table',
  strategy: Strategy.LOCAL,
  browser: false,
  args: [
    { name: 'endpoint', type: 'string', required: false, help: 'CDP endpoint (default: leafcli_CDP_ENDPOINT)' },
  ...
  func: async (kwargs) => {
    ...
    const wanted = process.env.leafcli_CDP_TARGET || '';
