...
cli({
    ...
    func: async (page) => {
        ...
        if (process.env.leafcli_VERBOSE) {
            console.error('[leafcli:v2ex] Navigating to /');
        }
    ...
    `);
        if (data.error) {
            if (process.env.leafcli_VERBOSE) {
                console.error(`[leafcli:v2ex:debug] Page Title: ${data.debug_title}`);
                console.error(`[leafcli:v2ex:debug] Page Body: ${data.debug_body}`);
            }
