 ...
 *
 * Usage: leafcli plugin create <name> [--dir <path>]
 *
 * Creates:
 *   <name>/
 *     leafcli-plugin.json   — manifest with name, version, description
 *     package.json          — ESM package with leafcli peer dependency
...
export function createPluginScaffold(name: string, opts: ScaffoldOptions = {}): ScaffoldResult {
  ...
  const manifest = {
    name,
    version: '0.1.0',
    description: opts.description ?? `An leafcli plugin: ${name}`,
    leafcli: `>=${PKG_VERSION}`,
  };
  writeFile(targetDir, 'leafcli-plugin.json', JSON.stringify(manifest, null, 2) + '\n');
  files.push('leafcli-plugin.json');

  // package.json
  const pkg = {
    name: `leafcli-plugin-${name}`,
    version: '0.1.0',
    type: 'module',
    description: opts.description ?? `An leafcli plugin: ${name}`,
    peerDependencies: {
      '@yangshengzhou/leafcli': `>=${PKG_VERSION}`,
    },
  };
...
`;
  ...
  // README.md
  const readme = `# leafcli-plugin-${name}

${opts.description ?? `An leafcli plugin: ${name}`}

## Install

\`\`\`bash
# From local development directory
leafcli plugin install file://${targetDir}

# From GitHub (after publishing)
leafcli plugin install github:<user>/leafcli-plugin-${name}
\`\`\`
...
leafcli list | grep ${name}

# Run a command
leafcli ${name} hello
leafcli ${name} greet --name World
\`\`\`
`;
