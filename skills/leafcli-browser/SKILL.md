---
name: leafcli-browser
description: Use when an agent needs to drive a real Chrome window via leafcli — inspect a page, fill forms, click through logged-in flows, or extract data ad-hoc. Covers the selector-first target contract, compound form fields, stale-ref handling, network capture, and the agent-native envelopes the CLI returns. Not for writing adapters — see leafcli-adapter-author for that.
allowed-tools: Bash(leafcli:*), Read, Edit, Write
---
...
This skill is for **driving a live browser** to accomplish an agent task. If you are building a reusable adapter under `~/.leafcli/clis/<site>/` use `leafcli-adapter-author` instead.
...
- `leafcli browser *` commands require a `<session>` positional immediately after `browser`. Use the same session name for a multi-step flow; use a different name to isolate parallel browser work.
...
```bash
leafcli browser gmail bind
leafcli browser gmail state
leafcli browser gmail click "Search"
leafcli browser gmail network
leafcli browser gmail unbind
```

Binding never owns the user window and never closes the user tab. It fails closed if the tab is closed or becomes non-debuggable. Re-run `leafcli browser <session> bind` when you switch to a different real tab.

Navigation is allowed on bound sessions because the session now represents explicit agent ownership of that tab. Tab mutation (`tab new`, `tab select`, `tab close`) is still blocked for bound sessions. Use an owned session when you want LeafCLI to manage tab lifecycle.

Bound sessions have no LeafCLI idle-close timer; the binding lasts until `unbind`, tab close, window close, or daemon restart.
...
2. **Prefer site adapters before raw browser driving.** If `leafcli <site> <command>` already covers the task, use that adapter command first (`leafcli facebook notifications`, `leafcli reddit read`, `leafcli chatgpt model <level>`, etc.). Use `leafcli browser ...` only for gaps, debugging, or one-off UI flows the adapter does not expose.
...
If `browser open` or `browser analyze` returns `sitemap.available: true`, switch to `leafcli-browser-sitemap` before continuing a multi-step site flow. The sitemap is prior context for pages, actions, workflows, APIs, and pitfalls; it is not truth. If the browser state disagrees with the sitemap, trust the browser and mark the sitemap stale via `leafcli-sitemap-author`.
...
List entries look like `{key, method, status, url, ct, size, shape, body_truncated?}`. Detail envelope is `{key, url, method, status, ct, size, shape, body, body_truncated?, body_full_size?, body_truncation_reason}`. Cache lives in `~/.leafcli/cache/browser-network/` so you can re-inspect without re-triggering the request.
...
```bash
leafcli browser hn open "https://news.ycombinator.com" \
  && leafcli browser hn state \
  && leafcli browser hn click 3
```
...
```bash
leafcli browser login open "https://example.com/login"
leafcli browser login state                          # find [N] for email, password, submit
leafcli browser login type 4 "me@example.com"
leafcli browser login type 5 "hunter2"
leafcli browser login get value 4                    # verify (autocomplete can eat chars)
leafcli browser login click 6                        # submit
leafcli browser login wait selector "[data-testid=account-menu]" --timeout 15000
leafcli browser login state                          # fresh refs on the logged-in page
```

### Pick from a long dropdown

```bash
leafcli browser form state                          # sidebar shows [12] <select name=country>
leafcli browser form find --css "select[name=country]"
# the compound.options_total is 137, but compound.current is "" — unselected.
leafcli browser form select 12 "Uruguay"
leafcli browser form get value 12                   # { value: "uy", match_level: "exact" }
```
...
```bash
leafcli browser mercury state                          # find category trigger ref
# If the trigger/option is not clear, use AX:
leafcli browser mercury state --source ax              # look for combobox/button/listbox/option names
leafcli browser mercury click 7                        # click category trigger
leafcli browser mercury state --source ax              # fresh refs after the portal/listbox opens
leafcli browser mercury click 12                       # click option
leafcli browser mercury get text 7                     # verify visible selected label
```
...
```bash
leafcli browser compare state --compare-sources
```
...
```bash
leafcli browser hn open "https://news.ycombinator.com"
leafcli browser hn network --filter "title,score"
# -> find the /topstories entry, note its key
leafcli browser hn network --detail topstories-a1b2
```

### Read a long article in chunks

```bash
leafcli browser article open "https://blog.example.com/long-post"
leafcli browser article extract --chunk-size 8000
# -> content + next_start_char: 8000
leafcli browser article extract --start 8000 --chunk-size 8000
...
```bash
leafcli browser checkout frames
# -> [{"index": 0, "url": "https://checkout.stripe.com/...", ...}]
leafcli browser checkout eval "(() => document.querySelector('input[name=cardnumber]')?.value)()" --frame 0
```
...
| symptom | fix |
|---------|-----|
| `leafcli doctor` red: "Browser not connected" | Start Chrome with `--remote-debugging-port=9222`, or install the extension from the [Chrome Web Store](https://chromewebstore.google.com/detail/leafcli/ildkmabpimmkaediidaifkhjpohdnifk). |
...
| Network cache seems stale | Bump `--ttl` down, or let it expire. The cache lives at `~/.leafcli/cache/browser-network/`. |

---

## See also

- `leafcli-adapter-author` — turning what you just figured out into a reusable `~/.leafcli/clis/<site>/<command>.js`.
- `leafcli-browser-sitemap` — consuming site sitemap context while driving a browser task.
- `leafcli-sitemap-author` — creating or updating sitemap knowledge when you discover a durable path or stale entry.
- `leafcli-autofix` — when an existing adapter breaks, this skill walks you through `--trace retain-on-failure` evidence and filing a fix.
