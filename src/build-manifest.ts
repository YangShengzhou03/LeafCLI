 ...
 *   - `leafcli twitter followers --help` rendered `Arguments:\n  user  ` with
...
async function main(): Promise<void> {
  ...
  if (fileURLToPath(import.meta.url).includes(`${path.sep}dist${path.sep}`)) {
    process.stderr.write(
      `[失败] Refusing to run build-manifest from dist/.\n`
  ...
  if (failures.length > 0) {
    process.stderr.write(`[失败] ${failures.length} adapter(s) failed to load:\n`);
  ...
  if (metadataIssues.length > 0) {
    process.stderr.write(
      `[失败] ${metadataIssues.length} positional arg(s) missing \`help\` text:\n`,
    );
    ...
    process.stderr.write(
      ...
      + `\`leafcli <site> <cmd> --help\` shows callers what the parameter is for.\n`
  ...
  if (existing) {
    ...
    if (removed.length > args.allowRemovals) {
      process.stderr.write(
      `[失败] ${removed.length} manifest entries would be removed; refusing to overwrite.\n`,
    );
  ...
  console.log(`[成功] Manifest compiled: ${entries.length} entries → ${OUTPUT}`);
}
