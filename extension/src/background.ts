/**
 * leafcli — Service Worker (background script).
 *
 * Connects to the leafcli daemon via WebSocket, receives commands,
 * dispatches them to Chrome APIs (debugger/tabs/cookies), returns results.
 */

declare const __leafcli_COMPAT_RANGE__: string;
...
const CONTEXT_ID_KEY = 'leafcli_context_id_v1';
...
async function connectAttempt(): Promise<void> {
  ...
  thisWs.onopen = () => {
    if (ws !== thisWs) return;
    console.log('[leafcli] Connected to daemon');
    ...
    safeSend(thisWs, {
      ...
      compatRange: __leafcli_COMPAT_RANGE__,
    });
  ...
  thisWs.onmessage = async (event) => {
    ...
    try {
      ...
      console.error('[leafcli] Message handling error:', err);
    }
  };
  ...
  thisWs.onclose = () => {
    ...
    if (ws !== thisWs) return;
    console.log('[leafcli] Disconnected from daemon');
    ws = null;
    scheduleReconnect();
  };
...
function getSessionName(session?: string): string {
  ...
  if (!raw) throw new CommandFailure(
    ...
    'Pass a browser session name, e.g. leafcli browser <session> <command>.',
  );
  return raw;
}
...
async function ensureOwnedContainerGroup(
  ...
  // a visible "leafcli Adapter" tab group. Its ownership anchors are the
...
async function ensureOwnedContainerGroupUnlocked(
  ...
  try {
    ...
    console.warn(`[leafcli] Failed to ensure ${role} tab group: ${err instanceof Error ? err.message : String(err)}`);
    throw err;
  }
}
...
async function ensureOwnedContainerWindowUnlocked(
  ...
  await persistRuntimeState();
  console.log(`[leafcli] Created owned ${role} window ${container.windowId} (start=${startUrl})`);
  ...
  if (existing) {
    if (!existing.owned) {
      throw new CommandFailure(
        ...
        `Session "${existing.session}" is bound to a user tab and does not own an leafcli tab lease.`,
...
chrome.windows.onRemoved.addListener(async (windowId) => {
  ...
  for (const [leaseKey, session] of automationSessions.entries()) {
    if (session.windowId === windowId) {
      console.log(`[leafcli] ${session.surface} container closed (session=${session.session})`);
      if (session.idleTimer) clearTimeout(session.idleTimer);
      automationSessions.delete(leaseKey);
      sessionOverrides.delete(leaseKey);
      scheduleIdleAlarm(leaseKey, IDLE_TIMEOUT_NONE);
...
chrome.tabs.onRemoved.addListener(async (tabId) => {
  ...
  for (const [leaseKey, session] of automationSessions.entries()) {
    if (session.preferredTabId === tabId) {
      ...
      sessionOverrides.delete(leaseKey);
      scheduleIdleAlarm(leaseKey, IDLE_TIMEOUT_NONE);
      console.log(`[leafcli] Session ${session.session} detached from tab ${tabId} (tab closed)`);
    }
  }
  await persistRuntimeState();
});
...
function initialize(): void {
  ...
  console.log('[leafcli] leafcli extension initialized');
}
...
async function resolveTab(tabId: number | undefined, leaseKey: string, initialUrl?: string): Promise<ResolvedTab> {
    ...
    try {
      ...
      if (session && !session.owned) {
        throw new CommandFailure(
          ...
          'Run "leafcli browser bind" again on a debuggable http(s) tab.',
        );
      }
      if (session && !matchesSession && session.preferredTabId === null && isDebuggableUrl(tab.url)) {
        ...
        console.warn(`[leafcli] Tab ${tabId} drifted to window ${tab.windowId}, moving back to ${session.windowId}`);
        try {
          ...
          console.warn(`[leafcli] Failed to move tab back: ${moveErr}`);
        }
      } else if (!isDebuggableUrl(tab.url)) {
        console.warn(`[leafcli] Tab ${tabId} URL is not debuggable (${tab.url}), re-resolving`);
      }
    } catch (err) {
      ...
      if (existingSession && !existingSession.owned) {
        ...
        throw new CommandFailure(
          ...
          'Run "leafcli browser bind" again, then retry the command.',
        );
      }
      console.warn(`[leafcli] Tab ${tabId} no longer exists, re-resolving`);
    }
  }
    ...
    try {
      ...
      if (!session.owned) {
        throw new CommandFailure(
          ...
          'Switch the tab to an http(s) page or run "leafcli browser bind" on another tab.',
        );
      }
    } catch (err) {
      if (err instanceof CommandFailure) throw err;
  ...
  // content in a window where an leafcli group may have disappeared.
  ...
  if (reuseTab?.id) {
    ...
    try {
      ...
      console.warn(`[leafcli] data: URI was intercepted (${updated.url}), creating fresh tab`);
    } catch {
...
async function handleNavigate(cmd: Command, leaseKey: string): Promise<Result> {
  ...
  await new Promise<void>((resolve) => {
    ...
    timeoutTimer = setTimeout(() => {
      timedOut = true;
      console.warn(`[leafcli] Navigate to ${targetUrl} timed out after 15s`);
      finish();
    }, 15000);
  });
  ...
  if (postNavigationSession && tab.windowId !== postNavigationSession.windowId) {
    console.warn(`[leafcli] Tab ${tabId} drifted to window ${tab.windowId} during navigation, moving back to ${postNavigationSession.windowId}`);
    try {
      await chrome.tabs.move(tabId, { windowId: postNavigationSession.windowId, index: -1 });
      tab = await chrome.tabs.get(tabId);
    } catch (moveErr) {
      console.warn(`[leafcli] Failed to recover drifted tab: ${moveErr}`);
    }
  }
...
async function handleTabs(cmd: Command, leaseKey: string): Promise<Result> {
  ...
  if (session && !session.owned && cmd.op !== 'list') {
    return {
      ...
      error: `Session "${session.session}" is bound to a user tab; tab new/select/close requires an owned leafcli session.`,
      errorHint: 'Unbind the session first, or use a different session for owned leafcli tabs.',
    };
  }
  switch (cmd.op) {
    case 'list': {
...
async function handleCdp(cmd: Command, leaseKey: string): Promise<Result> {
  ...
  try {
    ...
    const data = routeFrameId
      ? await executor.sendCommandInFrameTarget(tabId, routeFrameId, cmd.cdpMethod, stripleafcliFrameRoutingParams(params, true), aggressive, commandCdpTimeoutMs(cmd) ?? 30_000, routeTargetUrl)
      : await executor.sendDebuggerCommand(
        { tabId },
        cmd.cdpMethod,
        stripleafcliFrameRoutingParams(params, false),
...
function stripleafcliFrameRoutingParams(params: Record<string, unknown>, stripFrameId: boolean): Record<string, unknown> {
  ...
  if (session.owned) {
    ...
    if (tabId !== null) {
      ...
      if (hasOtherOwnedLease) {
        ...
        console.log(`[leafcli] Released owned tab lease ${tabId} (session=${session.session}, surface=${session.surface}, ${reason})`);
      } else {
        try {
          ...
          console.log(`[leafcli] Released owned tab lease ${tabId} as reusable placeholder (session=${session.session}, surface=${session.surface}, ${reason})`);
        } catch {
          await chrome.tabs.remove(tabId).catch(() => {});
          console.log(`[leafcli] Released owned tab lease ${tabId} (session=${session.session}, surface=${session.surface}, ${reason})`);
        }
      ...
      console.log(`[leafcli] Released legacy owned window lease ${session.windowId} without closing container (session=${session.session}, surface=${session.surface}, ${reason})`);
    }
  } else if (session.preferredTabId !== null) {
    await safeDetach(session.preferredTabId);
    console.log(`[leafcli] Detached borrowed tab lease ${session.preferredTabId} (session=${session.session}, surface=${session.surface}, ${reason})`);
  }
...
async function handleBind(cmd: Command, leaseKey: string): Promise<Result> {
  ...
  resetWindowIdleTimer(leaseKey);
  console.log(`[leafcli] Session ${getSessionFromKey(leaseKey)} explicitly bound to tab ${boundTab.id} (${boundTab.url})`);
