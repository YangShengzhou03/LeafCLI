...
const DEFAULT_CAPTURE_VAR = '__leafcli_ig_protocol_capture';
const DEFAULT_CAPTURE_ERRORS_VAR = '__leafcli_ig_protocol_capture_errors';
...
export function buildInstallInstagramProtocolCaptureJs(captureVar = DEFAULT_CAPTURE_VAR, captureErrorsVar = DEFAULT_CAPTURE_ERRORS_VAR) {
    ...
    (() => {
      ...
      XMLHttpRequest.prototype.open = function(method, url) {
        this.__leafcli_method = method;
        this.__leafcli_url = url;
        this.__leafcli_headers = {};
        return origOpen.apply(this, arguments);
      };
      XMLHttpRequest.prototype.setRequestHeader = function(name, value) {
        try {
          this.__leafcli_headers = this.__leafcli_headers || {};
          this.__leafcli_headers[String(name)] = String(value);
        } catch {}
        return origSetRequestHeader.apply(this, arguments);
      };
      XMLHttpRequest.prototype.send = function(body) {
        this.addEventListener('load', () => {
          if (!shouldCapture(this.__leafcli_url)) return;
          try {
            window[CAPTURE_VAR].push({
              kind: 'xhr',
              url: String(this.__leafcli_url || ''),
              method: String(this.__leafcli_method || 'GET').toUpperCase(),
              requestHeaders: this.__leafcli_headers || {},
...
export async function dumpInstagramProtocolCaptureIfEnabled(page) {
    if (process.env.leafcli_INSTAGRAM_CAPTURE !== '1')
        return;
