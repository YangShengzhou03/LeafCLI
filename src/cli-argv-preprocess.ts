/**
 * argv preprocessing: rewrite `leafcli browser <session> <subcommand> ...`
 * into `leafcli browser --session <session> <subcommand> ...` so commander
 ...
 * `leafcli --profile work browser foo state` is recognised as the `browser`
 ...
 * the literal word `browser` deeper in the argv (e.g. `leafcli adapter init
...
export function rewriteBrowserArgv(argv: readonly string[]): string[] {
  ...
  if (next === '--session' || next === '--session=' || next.startsWith('--session=')) {
    throw new BrowserSessionArgvError(
      'The `--session` flag is no longer a public option. Use the positional form: leafcli browser <session> <command>',
    );
  }
...
/**
 * `leafcli boss detail -abc123def` fails because commander parses
