 ...
 *   leafcli daemon status — show daemon state
 *   leafcli daemon stop   — graceful shutdown
 *   leafcli daemon restart — graceful shutdown, then start a fresh daemon
 */
...
export async function daemonStatus(): Promise<void> {
  ...
  if (status.extensionConnected) {
    ...
    extensionLabel = `${count} ${count === 1 ? 'profile' : 'profiles'} connected, none selected — run \`leafcli profile use <name>\``;
  } else if (status.profileDisconnected) {
    extensionLabel = 'requested profile not connected — run `leafcli profile use <name>`';
  } else {
  ...
  console.log(`Version: ${daemonVersion}${stale ? ` (CLI v${PKG_VERSION}; run: leafcli daemon restart)` : ''}`);
