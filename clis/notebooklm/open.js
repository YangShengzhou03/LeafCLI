...
import { CliError, EmptyResultError } from '@yangshengzhou/leafcli/errors';
...
cli({
    ...
    func: async (page, kwargs) => {
        ...
        if (state.kind !== 'notebook') {
            throw new CliError('NOTEBOOKLM_OPEN_FAILED', `NotebookLM notebook "${notebookId}" did not open in the adapter session`, 'Run `leafcli notebooklm list -f json` first and pass a valid notebook id.');
        }
        ...
        if (!current) {
            throw new EmptyResultError('leafcli notebooklm open', 'NotebookLM notebook metadata was not found after navigation.');
        }
        return [current];
    },
});
