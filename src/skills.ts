...
export interface leafcliSkillInfo {
  name: string;
  description: string;
  version: string;
  path: string;
...
export interface leafcliSkillReadResult {
  skill: string;
  path: string;
  content: string;
}
...
export function listleafcliSkills(packageRoot?: string): leafcliSkillInfo[] {
  const skillsRoot = getSkillsRoot(packageRoot);
  if (!fs.existsSync(skillsRoot)) return [];

  return fs.readdirSync(skillsRoot, { withFileTypes: true })
    .filter((entry) => entry.isDirectory() && entry.name.startsWith('leafcli-'))
    .map((entry) => readSkillInfo(skillsRoot, entry.name))
    .filter((entry): entry is leafcliSkillInfo => entry !== null)
    .sort((a, b) => a.name.localeCompare(b.name));
}

export function readleafcliSkill(target: string, relpath = '', packageRoot?: string): leafcliSkillReadResult {
  const { name, pathInSkill } = parseSkillTarget(target, relpath);
  if (!name.startsWith('leafcli-')) {
    throw new ArgumentError(`Unknown leafcli skill: ${name}`, 'Run "leafcli skills list" to see available leafcli skills.');
  }
  ...
  if (relativeToRoot.startsWith('..') || path.isAbsolute(relativeToRoot)) {
    throw new ArgumentError(`Invalid skill path: ${relativePath}`, 'Skill paths must stay inside the selected leafcli skill.');
  }
  if (!fs.existsSync(absolutePath) || !fs.statSync(absolutePath).isFile()) {
    throw new ArgumentError(`Skill file not found: ${name}/${relativePath}`, 'Run "leafcli skills list <skill>" is not supported yet; read SKILL.md or a known references/... file.');
  }
...
function readSkillInfo(skillsRoot: string, name: string): leafcliSkillInfo | null {
...
function normalizeSkillPath(raw: string): string {
  ...
  if (normalized.startsWith('/') || normalized.split('/').some((part) => part === '..')) {
    throw new ArgumentError(`Invalid skill path: ${raw}`, 'Use a path relative to an leafcli skill directory.');
  }
