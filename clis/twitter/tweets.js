...
function normalizeLimit(rawLimit) {
    ...
    if (!Number.isInteger(limit) || limit < 1 || limit > MAX_TWEETS_LIMIT) {
        throw new ArgumentError(
            ...
            `Example: leafcli twitter tweets @jack --limit 250`
        );
    }
    return limit;
}
...
function normalizePageDelaySeconds(rawDelay) {
    ...
    if (!Number.isInteger(delay) || delay < 0 || delay > 60) {
        throw new ArgumentError(
            ...
            'Example: leafcli twitter tweets @jack --limit 250 --page-delay 2'
        );
    }
    return delay;
}
...
cli({
    ...
    func: async (page, kwargs) => {
        ...
        if (rawUsername && !username) {
            throw new ArgumentError('twitter tweets username must be a valid Twitter/X handle', 'Example: leafcli twitter tweets @jack --limit 20');
        }
