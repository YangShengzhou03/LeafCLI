...
import { ArgumentError, CommandExecutionError, EmptyResultError, getErrorMessage } from '@yangshengzhou/leafcli/errors';
...
function normalizeLimit(value) {
    ...
    if (!Number.isInteger(limit) || limit <= 0) {
        throw new ArgumentError('limit must be a positive integer', `Example: leafcli aibase news --limit ${DEFAULT_LIMIT}`);
    }
    if (limit > MAX_LIMIT) {
        throw new ArgumentError(`limit must be <= ${MAX_LIMIT}`, `Example: leafcli aibase news --limit ${MAX_LIMIT}`);
    }
    return limit;
}
