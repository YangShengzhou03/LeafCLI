...
cli({
    ...
    func: async (page, kwargs) => {
        ...
        if (candidates.length === 0) {
            return [{ name: '-', status: '[警告] 无候选人', detail: '当前没有待招呼的推荐候选人' }];
        }
        const results = [];
        for (const candidate of candidates) {
            ...
            try {
                ...
                if (!clicked) {
                    results.push({ name: friendName, status: '[失败] 跳过', detail: '在聊天列表中未找到' });
                    continue;
                }
                await page.wait({ time: 2 });
                const sent = await typeAndSendMessage(page, text);
                if (!sent) {
                    results.push({ name: friendName, status: '[失败] 失败', detail: '找不到消息输入框' });
                    continue;
                }
                await page.wait({ time: 1.5 });
                results.push({ name: friendName, status: '[成功] 已发送', detail: text });
            }
            catch (e) {
                results.push({ name: friendName, status: '[失败] 失败', detail: e.message?.substring(0, 80) || '未知错误' });
            }
        }
        return results;
    },
