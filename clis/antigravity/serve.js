 ...
 *
 * Usage:
 *   leafcli antigravity serve --port 8082
 *   ANTHROPIC_BASE_URL=http://localhost:8082 claude
 */
import { createServer } from 'node:http';
import { CDPBridge } from '@yangshengzhou/leafcli/browser/cdp';
import { resolveElectronEndpoint } from '@yangshengzhou/leafcli/launcher';
import { EXIT_CODES, getErrorMessage } from '@yangshengzhou/leafcli/errors';
// ─── Helpers ─────────────────────────────────────────────────────────
...
export async function startServe(opts = {}) {
    ...
    const envTimeoutSeconds = parseEnvTimeout('leafcli_ANTIGRAVITY_TIMEOUT', 120);
    ...
    async function ensureConnected() {
        ...
        // If multiple editor windows are open, set leafcli_CDP_TARGET to the window title.
        if (process.env.leafcli_CDP_TARGET) {
            console.error(`[serve] Using leafcli_CDP_TARGET=${process.env.leafcli_CDP_TARGET}`);
        }
        ...
        console.error(`[serve] Connecting via CDP (target pattern: "${process.env.leafcli_CDP_TARGET}")...`);
        cdp = new CDPBridge();
        try {
        ...
        console.error('[serve] [成功] CDP connected.');
        // Quick verification
    ...
    `);
        if (!hasUI) {
            console.error('[serve] [警告] Warning: chat UI elements not found in this target. Try setting leafcli_CDP_TARGET to the correct window title.');
        }
        return page;
    }
    ...
    server.listen(port, '127.0.0.1', () => {
        console.error(`\n[serve] [成功] Antigravity API proxy running at http://127.0.0.1:${port}`);
