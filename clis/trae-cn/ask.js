...
import { TimeoutError } from '@yangshengzhou/leafcli/errors';
import {
  activityScript,
  approveTraePrompts,
  isActivityComplete,
...
export const askCommand = cli({
  ...
  example: 'leafcli_CDP_ENDPOINT=http://127.0.0.1:39240 leafcli_CDP_TARGET=talk leafcli trae-cn ask "请只回复 OK" --timeout 120 -f json',
  domain: 'localhost',
    ...
    if (latest?.Text) {
      ...
      if (blocked) {
        throw new TimeoutError(
          'trae-cn ask',
          timeout,
          `Current status=${latestActivity?.Status ?? ''}, approvalPending=${latestActivity?.ApprovalPending ?? ''}, approvalKind=${latestActivity?.ApprovalKind ?? ''}, approvalButton=${latestActivity?.ApprovalButton ?? ''}. Use "leafcli trae-cn targets -f json" to find the waiting target, then run "leafcli_CDP_TARGET=<target> leafcli trae-cn watch --stream true --duration 300" or "leafcli trae-cn approve".`,
        );
      }
      return [latest];
    }
