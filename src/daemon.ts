/**
 * leafcli micro-daemon — HTTP + WebSocket bridge between CLI and Chrome Extension.
 *
 * Architecture:
 ...
 *   2. Custom header — require X-leafcli header (browsers can't send it
 ...
 *
 * Lifecycle:
 *   - Auto-spawned by leafcli on first browser command
...
if (!isIgnorableDaemonPortEnv(process.env.leafcli_DAEMON_PORT)) {
  log.error(unsupportedDaemonPortEnvMessage(process.env.leafcli_DAEMON_PORT));
...
function resolveExtensionConnection(contextId?: string, preferredContextId?: string): {
  ...
  if (route.fallbackFrom && !staleDefaultWarned.has(route.fallbackFrom)) {
    ...
    log.warn(
      ...
      `using the only connected profile "${route.contextId}". Update the default with: leafcli profile use <name>`,
    );
  }
...
async function handleRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  // Health-check endpoint — no X-leafcli header required.
