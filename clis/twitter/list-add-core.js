...
export async function listAddUser(page, kwargs) {
        ...
        if (!listId || !/^\d+$/.test(listId)) {
            throw new ArgumentError(`Invalid listId: ${JSON.stringify(kwargs.listId)}. Expected numeric ID.`, 'Example: leafcli twitter list-add 123456789 alice');
        }
        if (!username) {
            throw new ArgumentError('twitter list-add username is required', 'Example: leafcli twitter list-add 123456789 alice');
        }
        ...
        // leafcli >=1.7.x wraps page.evaluate return values as { session, data }.
        ...
        // Don't unwrap listsData: leafcli spreads GraphQL response to top-level + adds session;
