...
import { saveBase64ToFile } from '@yangshengzhou/leafcli/utils';
...
export const imageCommand = cli({
    ...
    func: async (page, kwargs) => {
        ...
        if (!urls.length) {
            return [{ status: '[警告] no-images', file: '[文件] -', link: `[链接] ${link}` }];
        }
        if (skipDownload) {
            return [{ status: '[已生成] generated', file: '[文件] -', link: `[链接] ${link}` }];
        }
        const assets = await exportGeminiImages(page, urls);
        if (!assets.length) {
            return [{ status: '[警告] export-failed', file: '[文件] -', link: `[链接] ${link}` }];
        }
        ...
        for (let index = 0; index < assets.length; index += 1) {
            ...
            results.push({ status: '[成功] saved', file: `[文件] ${displayPath(filePath)}`, link: `[链接] ${link}` });
        }
        return results;
    },
});
