---
name: leafcli-sitemap-author
description: Guide for using LeafCLI sitemaps to navigate websites efficiently. Includes site navigation, page states, actions, workflows, and recovery strategies when pages change. Use when exploring new websites or when site behavior differs from expectations.
allowed-tools: Bash(leafcli:*), Read, Edit, Write, Grep
---

# leafcli-sitemap-author

You are authoring a **task execution graph for agents**, not an SEO sitemap. The artifact should help an agent using `leafcli browser` decide where it is, what path to take next, which LeafCLI adapter to prefer, and how to recover when the page disagrees with memory.
...
- **Local overlay**: `~/.leafcli/sites/<site>/sitemap/`
...
2. **Verify reality**: use `leafcli browser <session> state`, `find`, `network`, and `analyze`; browser state is truth. If you just completed an `leafcli-adapter-author` session for this site, start from the retained browse trace under `~/.leafcli/sites/<site>/traces/` as seed evidence instead of re-discovering the path from zero.
...
5. **Write local draft**: update `~/.leafcli/sites/<site>/sitemap/...` unless explicitly promoting to repo.
...
evidence: leafcli browser <cmd> or trace:<path>
```
...
- **Best path**: prefer existing `leafcli <site> <command>` adapter if it covers the goal.
...
```yaml
on_adapter_fail:
  - adapter_health_update: leafcli twitter post -> suspect
  - leafcli browser state (verify current page)
...
- Drafts go inside `sitemap/draft-<topic>.md`, not `~/.leafcli/sites/<site>/sitemap.draft.md` at the parent level — the latter is invisible to `leafcli browser` sitemap availability detection.
