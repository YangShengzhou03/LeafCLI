import { ArgumentError, AuthRequiredError, CommandExecutionError, ConfigError } from '@yangshengzhou/leafcli/errors';
...
export function dispatchEvaluateResult(r) {
  switch (r && r.kind) {
    ...
    case 'no-server':
      throw new ConfigError(r.detail, 'Run `leafcli slock server-use <slug>` to set the active server.');
    case 'unresolvable':
