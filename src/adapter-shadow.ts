...
export function findShadowedUserAdapters(opts: AdapterShadowOptions = {}): AdapterShadow[] {
  const userClisDir = opts.userClisDir ?? path.join(os.homedir(), '.leafcli', 'clis');
...
export function formatAdapterShadowIssue(shadows: AdapterShadow[]): string {
  ...
  lines.push('Remove the local ~/.leafcli/clis copy, or run leafcli adapter reset <site>, when you want packaged updates.');
  return lines.join('\n');
}
