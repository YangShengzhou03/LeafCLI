...
import { BROWSER_JSON_SNIFF_FN, throwIfLoginWall } from '@yangshengzhou/leafcli/utils';
...
cli({
    ...
    browser: true,
    args: [
        { name: 'listId', positional: true, type: 'string', required: true, help: 'Numeric ID of a Twitter/X list (e.g. from `leafcli twitter lists`)' },
    ...
    func: async (page, kwargs) => {
        ...
        if (!listId || !/^\d+$/.test(listId)) {
            throw new CommandExecutionError(`Invalid listId: ${JSON.stringify(kwargs.listId)}. Expected a numeric ID (see \`leafcli twitter lists\`).`);
        }
        ...
        // leafcli >=1.7.x wraps primitive page.evaluate returns as { session, data: <value> }.
