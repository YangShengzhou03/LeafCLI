 ...
 *
 * Plugins live in ~/.leafcli/plugins/<name>/.
 * Monorepo clones live in ~/.leafcli/monorepos/<repo-name>/.
...
export function getLockFilePath(): string {
  return path.join(getHomeDir(), '.leafcli', 'plugins.lock.json');
}

/** Monorepo clones directory: ~/.leafcli/monorepos/ */
export function getMonoreposDir(): string {
  return path.join(getHomeDir(), '.leafcli', 'monorepos');
}
...
export interface PluginInfo {
  ...
  monorepoName?: string;
  /** Description from leafcli-plugin.json. */
  description?: string;
}
 ...
 * filesystems (e.g. /tmp → ~/.leafcli). In that case we copy then remove.
 */
...
function cloneRepoToTemp(cloneUrl: string): string {
  const tmpCloneDir = path.join(
    os.tmpdir(),
    `leafcli-clone-${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}`,
  );
...
export function validatePluginStructure(pluginDir: string): ValidationResult {
  ...
  if (hasTs) {
    ...
    if (!fs.existsSync(pkgJsonPath)) {
      errors.push('Plugin contains .ts files but no package.json. A package.json with "type": "module" and "@yangshengzhou/leafcli" peer dependency is required for TS plugins.');
    } else {
      try {
...
function finalizePluginRuntime(pluginDir: string): void {
  // Symlink host leafcli so TS plugins resolve '@yangshengzhou/leafcli/registry'
  // against the running host, not a stale npm-published version.
  linkHostleafcli(pluginDir);
...
export function installPlugin(source: string): string | string[] {
  ...
  return withTempClone(parsed.cloneUrl!, (tmpCloneDir) => {
    ...
    if (manifest?.leafcli && !checkCompatibility(manifest.leafcli)) {
      throw new Error(
        `Plugin requires leafcli ${manifest.leafcli}, but current version is incompatible.`
      );
    }
...
function installSinglePlugin(
  ...
  if (fs.existsSync(targetDir)) {
    throw new PluginError(`Plugin "${pluginName}" is already installed at ${targetDir}`, 'Use "leafcli plugin uninstall" first, or pick a different name.');
  }
...
function installLocalPlugin(localPath: string, name: string): string {
  ...
  if (manifest?.leafcli && !checkCompatibility(manifest.leafcli)) {
    throw new PluginError(
      `Plugin requires leafcli ${manifest.leafcli}, but current version is incompatible.`,
      'Upgrade leafcli to a compatible version.',
    );
  }
...
function installMonorepo(
  ...
  for (const { name, entry } of pluginsToInstall) {
    ...
    if (entry.leafcli && !checkCompatibility(entry.leafcli)) {
      log.warn(`Skipping "${name}": requires leafcli ${entry.leafcli}`);
      continue;
    }
...
function collectUpdatedMonorepoPlugins(
  ...
  for (const [pluginName, entry] of Object.entries(lock)) {
    ...
    if (manifestEntry.leafcli && !checkCompatibility(manifestEntry.leafcli)) {
      throw new Error(`Sub-plugin "${pluginName}" requires leafcli ${manifestEntry.leafcli}`);
    }
 ...
 * Reads leafcli-plugin.json for description/version when available.
 */
...
function parseSource(
  ...
  if (source.startsWith('file://')) {
    try {
      ...
      return {
        type: 'local',
        localPath,
        name: path.basename(localPath).replace(/^leafcli-plugin-/, ''),
      };
    } catch {
      return null;
    }
  ...
  if (githubSubMatch) {
    ...
    const name = repo.replace(/^leafcli-plugin-/, '');
    return {
      type: 'git',
  ...
  if (sshUrlMatch) {
    ...
    const name = repoSegment.replace(/^leafcli-plugin-/, '');
...
/**
 * Symlink the host leafcli package into a plugin's node_modules.
 * This ensures TS plugins resolve '@yangshengzhou/leafcli/registry' against
 * the running host installation rather than a stale npm-published version.
 */
function linkHostleafcli(pluginDir: string): void {
  try {
    const hostRoot = resolveHostleafcliRoot();

    const targetLink = path.join(pluginDir, 'node_modules', '@yangshengzhou', 'leafcli');
    ...
    log.debug(`Linked host leafcli into plugin: ${targetLink} → ${hostRoot}`);
  } catch (err) {
    log.warn(`Failed to link host leafcli into plugin: ${getErrorMessage(err)}`);
  }
}
...
export function resolveEsbuildBin(): string | null {
  ...
  try {
    const lookupCmd = isWindows ? 'where esbuild' : 'where esbuild';
...
function resolveHostleafcliRoot(startFile = fileURLToPath(import.meta.url)): string {
  ...
  while (true) {
    ...
    if (fs.existsSync(pkgPath)) {
      try {
        const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf-8'));
        if (pkg?.name === '@yangshengzhou/leafcli') {
          return dir;
        }
      } catch {
 ...
 * Uses esbuild from the host leafcli's node_modules for fast single-file transpilation.
 */
  ...
  try {
    ...
    if (!esbuildBin) {
      log.warn(
        ...
        'Install esbuild (`npm i -g esbuild`) or ensure it is available in the leafcli host node_modules.'
      );
      return;
    }
...
export {
  resolveHostleafcliRoot as _resolveHostleafcliRoot,
