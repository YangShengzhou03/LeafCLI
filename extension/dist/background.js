...
async function ensureAttached(tabId, aggressiveRetry = false) {
  ...
  for (let attempt = 1; attempt <= MAX_ATTACH_RETRIES; attempt++) {
    try {
      ...
      if (attempt < MAX_ATTACH_RETRIES) {
        console.warn(`[leafcli] attach attempt ${attempt}/${MAX_ATTACH_RETRIES} failed: ${lastError}, retrying in ${RETRY_DELAY_MS}ms...`);
  ...
  if (lastError) {
    ...
    console.warn(`[leafcli] attach failed for tab ${tabId}: url=${finalUrl}, windowId=${finalWindowId}, error=${lastError}`);
...
const JOURNAL_KEY = "leafcli_command_journal_v1";
...
async function handleCdp(cmd, leaseKey) {
  ...
  try {
    ...
    const data = routeFrameId ? await sendCommandInFrameTarget(tabId, routeFrameId, cmd.cdpMethod, stripleafcliFrameRoutingParams(params, true), aggressive, commandCdpTimeoutMs(cmd) ?? 3e4, routeTargetUrl) : await sendDebuggerCommand(
      { tabId },
      cmd.cdpMethod,
...
function stripleafcliFrameRoutingParams(params, stripFrameId) {
