 ...
 *
 * Usage:
 *   leafcli twitter download elonmusk --limit 50 --output ./twitter
 *   leafcli twitter download --tweet-url https://x.com/xxx/status/123 --output ./twitter
 */
import { cli, Strategy } from '@yangshengzhou/leafcli/registry';
import { ArgumentError, AuthRequiredError, CliError, CommandExecutionError, EmptyResultError } from '@yangshengzhou/leafcli/errors';
import { formatCookieHeader } from '@yangshengzhou/leafcli/download';
import { downloadMedia } from '@yangshengzhou/leafcli/download/media-download';
import {
...
cli({
    ...
    func: async (page, kwargs) => {
        try {
            ...
            if (!username) {
                throw new ArgumentError('twitter download username must be a valid Twitter/X handle', 'Example: leafcli twitter download @jack --limit 20');
            }
