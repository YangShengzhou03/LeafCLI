  ...
  switch (kind) {
    case 'daemon-not-running':
      return new BrowserConnectError(
        'Cannot connect to leafcli daemon.' + (detail ? `\n\n${detail}` : ''),
        `Run \`leafcli doctor\` to diagnose, or \`leafcli daemon restart\` to force a fresh daemon. Default port is ${DEFAULT_DAEMON_PORT}.`,
        kind,
      );
