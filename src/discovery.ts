...
/** User runtime directory: ~/.leafcli */
export const USER_leafcli_DIR = path.join(os.homedir(), '.leafcli');
/** User CLIs directory: ~/.leafcli/clis */
export const USER_CLIS_DIR = path.join(USER_leafcli_DIR, 'clis');
/** Plugins directory: ~/.leafcli/plugins/ */
export const PLUGINS_DIR = path.join(USER_leafcli_DIR, 'plugins');
...
/**
 * Ensure ~/.leafcli/node_modules/@yangshengzhou/leafcli symlink exists so that
 * user CLIs in ~/.leafcli/clis/ can `import { cli } from '@yangshengzhou/leafcli/registry'`.
 *
 * This is the sole resolution mechanism — adapters use package exports
 * (e.g. `@yangshengzhou/leafcli/registry`, `@yangshengzhou/leafcli/errors`) and
 * Node.js resolves them through this symlink.
 */
export async function ensureUserCliCompatShims(baseDir: string = USER_leafcli_DIR): Promise<void> {
  await fs.promises.mkdir(baseDir, { recursive: true });

  // package.json for ESM resolution in ~/.leafcli/
  const pkgJsonPath = path.join(baseDir, 'package.json');
  const pkgJsonContent = `${JSON.stringify({ name: 'leafcli-user-runtime', private: true, type: 'module' }, null, 2)}\n`;
  try {
  ...
  // Create node_modules/@yangshengzhou/leafcli symlink pointing to the installed package root.
  const leafcliRoot = PACKAGE_ROOT;
  const symlinkDir = path.join(baseDir, 'node_modules', '@yangshengzhou');
  const symlinkPath = path.join(symlinkDir, 'leafcli');
  try {
    let needsUpdate = true;
    try {
      const existing = await fs.promises.readlink(symlinkPath);
      if (existing === leafcliRoot) needsUpdate = false;
    } catch { /* doesn't exist */ }
    if (needsUpdate) {
      ...
      await fs.promises.symlink(leafcliRoot, symlinkPath, 'junction');
    }
  } catch (err) {
 ...
 *
 * With smart sync, ~/.leafcli/clis/ only holds files that differ from the
...
/**
 * Discover and register plugins from ~/.leafcli/plugins/.
  ...
  await Promise.all(files.map(async (file) => {
    ...
    if (file.endsWith('.js') && !file.endsWith('.d.js')) {
      ...
      log.warn(
        ...
        `Run "leafcli plugin update ${site}" to re-transpile, or install esbuild.`
      );
    }
  }));
}
