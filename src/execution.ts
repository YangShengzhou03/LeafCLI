...
const _userClisDir = `${os.homedir()}/.leafcli/clis/`;
  ...
  try {
    if (shouldUseBrowserSession(cmd)) {
      ...
      if (electron) {
        ...
        const manualEndpoint = process.env.leafcli_CDP_ENDPOINT;
        if (manualEndpoint) {
      ...
      // Requirement vs preference: --profile / leafcli_PROFILE route strictly;
...
function exportTraceArtifact(
  ...
  try {
    ...
    if (status === 'failure' && error !== undefined) {
      ...
      process.stderr.write(`leafcli trace artifact: ${trace.dir}\n`);
    }
    try {
      onTraceExport?.(trace);
    } catch (err) {
...
function resolveBrowserWindowMode(defaultMode: BrowserWindowMode = 'background', rawOption?: unknown): BrowserWindowMode {
  return normalizeWindowMode('--window', rawOption)
    ?? normalizeWindowMode('leafcli_WINDOW', process.env.leafcli_WINDOW)
    ?? defaultMode;
}
