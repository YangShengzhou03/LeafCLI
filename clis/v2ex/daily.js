 ...
 */
import { CommandExecutionError } from '@yangshengzhou/leafcli/errors';
...
cli({
    ...
    func: async (page) => {
        ...
        if (process.env.leafcli_VERBOSE) {
            console.error('[leafcli:v2ex] Navigating to /mission/daily');
        }
        ...
        for (let i = 0; i < 5; i++) {
            ...
            if (!title?.includes('Just a moment'))
                break;
            if (process.env.leafcli_VERBOSE)
                console.error('[leafcli:v2ex] Waiting for Cloudflare...');
        }
    ...
    `);
        if (checkResult.error) {
            if (process.env.leafcli_VERBOSE) {
                console.error(`[leafcli:v2ex:debug] Page Title: ${checkResult.debug_title}`);
                console.error(`[leafcli:v2ex:debug] Page Body: ${checkResult.debug_body}`);
            }
            throw new CommandExecutionError(checkResult.error);
        }
        if (checkResult.claimed) {
            return [{ status: '[成功] 已签到', message: checkResult.message }];
        }
        // Perform check in
        if (process.env.leafcli_VERBOSE) {
            console.error(`[leafcli:v2ex] Found check-in token: once=${checkResult.once}. Checking in...`);
        }
    ...
    `);
        if (verifyResult.success) {
            return [{ status: '[成功] 签到成功', message: `当前余额: ${verifyResult.balance || '未知'}` }];
        }
        else {
            return [{ status: '[失败] 签到失败', message: '未能确认签到结果，请手动检查' }];
        }
    },
});
