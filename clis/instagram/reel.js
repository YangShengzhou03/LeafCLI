...
import { Page as BrowserPage } from '@yangshengzhou/leafcli/browser/page';
...
function buildSafeTempVideoPath(filePath) {
    ...
    return path.join(os.tmpdir(), `leafcli-instagram-video-real${ext}`);
}
    ...
    (() => {
      ...
      for (const root of roots) {
        ...
        for (const input of inputs) {
          ...
          input.setAttribute('data-leafcli-reel-upload-index', String(index));
          selectors.push('[data-leafcli-reel-upload-index="' + index + '"]');
          index += 1;
        }
      }
