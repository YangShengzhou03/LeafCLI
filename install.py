...
def print_header():
    ...
    print()
    print(f"  {PRIMARY}{BOLD}LeafCLI{RST}")
    print(f"  {DIM}{'~' * 40}{RST}")
    print(f"  {SECONDARY}让 AI 帮你操作浏览器，一句话搞定重复操作{RST}")
    print()

def print_step(current: int, total: int, desc: str):
    print()
...
def print_diagnosis(error_msg: str):
...
class Spinner:
    ...
    def stop(self, success: bool = True, final: str = None):
        self.running = False
        if self.thread:
            self.thread.join(timeout=0.5)
        print(f"\r{' ' * (len(self.msg) + 15)}", end='\r')
        if final:
...
def select_best_npm_mirror() -> Tuple[str, str]:
    ...
    results = {}
    for name, url in NPM_MIRRORS.items():
        try:
            start = time.time()
...
# ==================== 命令执行 ====================
def run_command(command: str, show_output: bool = True, timeout: int = 300) -> Tuple[bool, str]:
    """运行命令，返回 (成功, 输出)"""
    try:
        ...
        result = subprocess.run(command, shell=True, check=True, capture_output=True,
                              text=True, encoding=enc, errors='replace', timeout=timeout)
...
# ==================== 依赖检查 ====================
def check_dependency(name: str, cmd: str, version_re: str = None,
                    min_ver: int = None) -> Tuple[bool, str]:
    info(f"正在检查 {name}...")
    try:
        enc = 'utf-8' if platform.system() == "Windows" else None
        r = subprocess.run(cmd, shell=True, capture_output=True, text=True,
                          check=True, encoding=enc, errors='replace', timeout=10)
        ver = r.stdout.strip()
...
AI_TOOLS = {
    ...
    'cline':       ('Cline',        ['.cline'], None),
    'copilot':     ('GitHub Copilot',['.copilot', '.github/copilot'], None),
    'gemini':      ('Gemini CLI',   ['.gemini'], None),
    'aider':       ('Aider',        ['.aider'], None),
    'zed':         ('Zed AI',       ['.zed', '.config/zed'], None),
}
...
def detect_ai_tools() -> List[Dict]:
    ...
    with ThreadPoolExecutor(max_workers=5) as pool:
        for r in pool.map(lambda args: check(*args), AI_TOOLS.items()):
            if r['detected']:
                found.append(r)
                ok(r['name'])
...
def main():
    ...
    if args.resume:
        p = load_progress()
        if p and (args.silent or ask_user("是否从上次中断处继续")):
            info(f"从 {p['step']} 继续")
    ...
    # 2. 安装 LeafCLI
    print_step(2, 4, "安装 LeafCLI")
    if args.silent or ask_user("是否自动安装 LeafCLI"):
        if not install_package("LeafCLI", "npm install -g @yangshengzhou/leafcli"):
            error("请手动运行: npm install -g @yangshengzhou/leafcli")
            sys.exit(1)
        save_progress("leafcli_installed")
    else:
        info("请手动运行: npm install -g @yangshengzhou/leafcli")
        sys.exit(0)
    ...
    # 4. 安装 Skills
    print_step(4, 4, "为 AI 工具安装 Skills")
    ...
    if tools:
        ...
        if args.silent or ask_user("确认安装 LeafCLI Skills"):
            ...
            success, _ = run_command(cmd, show_output=True, timeout=180)
            if success:
                ok("Skills 安装完成")
            else:
                ok("Skills 已安装到全局目录")
