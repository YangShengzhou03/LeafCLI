...
export async function ensureBrowserBridgeReady(
  ...
  if (isStale) {
    ...
    if (verbose && (process.env.leafcli_VERBOSE || process.stderr.isTTY)) {
      process.stderr.write(`Stale daemon detected (${reason}). Restarting...\n`);
    }
    ...
    if (!portReleased) {
      throw new BrowserConnectError(
        ...
        '  Run manually: leafcli daemon stop',
        'daemon-not-running',
      );
    }
  ...
  } else if (verbose && (process.env.leafcli_VERBOSE || process.stderr.isTTY)) {
    process.stderr.write('Waiting for Chrome/Chromium extension to connect...\n');
    process.stderr.write('   Make sure Chrome or Chromium is open and the leafcli extension is enabled.\n');
  }
...
function browserConnectErrorFromHealth(health: DaemonHealth, contextId?: string): BrowserConnectError {
  if (health.state === 'profile-required') {
    return new BrowserConnectError(
      ...
      'Select one with --profile <name>, leafcli_PROFILE=<name>, or leafcli profile use <name>.\n' +
      'Run leafcli profile list to see connected profiles.',
      'profile-required',
    );
  }
  if (health.state === 'profile-disconnected') {
    ...
    return new BrowserConnectError(
      ...
      'Open the matching Chrome profile and make sure the leafcli extension is enabled, or choose another profile with leafcli profile use <name>.',
  ...
  return new BrowserConnectError(
    'Failed to start leafcli daemon',
