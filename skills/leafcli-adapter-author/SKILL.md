---
name: leafcli-adapter-author
description: Use when writing an LeafCLI adapter for a new site or adding a new command to an existing site. Guides end-to-end from first recon through field decoding, adapter coding, and verify. Replaces leafcli-oneshot / leafcli-explorer. For ad-hoc browser driving (no adapter), see leafcli-browser instead; for a top-level orientation to leafcli, see leafcli-usage.
allowed-tools: Bash(leafcli:*), Read, Edit, Write, Grep
---

# leafcli-adapter-author

你是要给一个站点写 adapter 的 agent。这份 skill 目标：**从零到通过 `leafcli browser verify` 的 30 分钟内闭环**。

全程用现有工具：`leafcli browser *` / `leafcli doctor` / `leafcli browser init` / `leafcli browser verify`。没有新命令。
...
| `PAGE_FETCH` | internal-unstable | 只能在页面上下文 `fetch` 才能复用 same-origin/session/runtime | `leafcli browser eval fetch(...)` 非空；必须解释为什么避不开内部接口 |
...
START
  ...
  ▼
┌──────────────────────────┐
│ leafcli doctor 通？      │── no ──→ 修桥接（doctor 输出里的提示）
└──────────────────────────┘
  │ yes
  ▼
┌────────────────────────────────────────────────────┐
│ 读站点记忆：                                        │
│   1. ~/.leafcli/sites/<site>/endpoints.json         │
│   2. ~/.leafcli/sites/<site>/notes.md               │
...
└──────────────────────────┘
  ...
  ▼
┌──────────────────────────┐
│ leafcli browser init      │  生成 ~/.leafcli/clis/<site>/<name>.js 骨架
│ 复制最像的邻居 adapter    │
│ 改 name / URL / 映射三处  │
└──────────────────────────┘
  │
  ▼
┌──────────────────────────┐
│ leafcli browser verify    │── 失败 ──→ autofix skill，用 --trace retain-on-failure 回对应步骤
└──────────────────────────┘
  │ 成功
  ▼
┌──────────────────────────┐
...
└──────────────────────────┘
  ...
  ▼
┌──────────────────────────┐
│ 回写 ~/.leafcli/sites/   │  endpoints / field-map / notes / fixtures
└──────────────────────────┘
  │
  ▼
DONE
...
```
[ ] 1. leafcli doctor 返回 "Everything looks good"
[ ] 2. 读站点记忆：
       [ ] ~/.leafcli/sites/<site>/endpoints.json 存在？里面有想要的 endpoint？
...
[ ] 3. 侦察（site-recon.md）：
       [ ] **首选**：`leafcli browser analyze <url>` 一步拿 pattern + 反爬 + 最近 adapter + next step
...
[ ] 9. 写 adapter（adapter-template.md）：
       [ ] leafcli browser init <site>/<name>
       [ ] 找同站点或同类型最像的 adapter，cp 过来
       [ ] 改 name / URL / 字段映射
[ ] 10. leafcli browser verify <site>/<name>
        [ ] 首轮通过后立刻 `--write-fixture` 生成 `~/.leafcli/sites/<site>/verify/<cmd>.json` 种子
        [ ] 手改种子：加 `patterns`（URL / 日期 / ID 格式）+ `notEmpty`（核心字段）+ 收紧 `rowCount`
        [ ] 再跑一次 `leafcli browser verify <site>/<name>`，确认 ✓ matches fixture
...
[ ] 12. 回写站点记忆（**verify 通过 + 肉眼比对对得上之后**，schema 见 `references/site-memory.md`）：
        ...
        [ ] `verify/<cmd>.json`：**必填。** `leafcli browser verify` 的期望值（args / rowCount / columns / types / patterns / notEmpty），Step 10 已经让你生成了，这里只是 checklist
        [ ] `fixtures/<cmd>-<YYYYMMDDHHMM>.json`：存一份该 endpoint 的完整响应样本（去掉 cookie / token / 用户私有字段再存），给后续字段对比 / 离线 replay 用
        [ ] 调试过程中如果在 repo / adapter 目录 dump 过临时文件（`.dbg-*.html` / `raw-*.json` / 等），**在 commit 前清干净**——这些本来就该落在 `~/.leafcli/sites/<site>/fixtures/` 或 `/tmp/`
```
...
| `references/site-memory.md` | 总览：in-repo 种子 + 本地 `~/.leafcli/sites/` 的两层结构 |
...
- adapter 只引 `@yangshengzhou/leafcli/registry` + `@yangshengzhou/leafcli/errors`，不用第三方
...
- 写私人 adapter 用 `~/.leafcli/clis/<site>/<name>.js`（免 build）；要提 PR 才 copy 到 `clis/<site>/<name>.js`
- 站点记忆每轮回写：没记忆 → 用 skill → 产生记忆 → 下次变 5 分钟
- **调试过程中的原始 dump / 抓包 / HTML 样本只能落在 `~/.leafcli/sites/<site>/fixtures/` 或 `/tmp/`。严禁在 repo 根目录、`clis/<site>/` 或当前工作目录留 `.dbg-*.html / raw-*.json / sample.*` 这类临时文件**（PR diff 会带上去，别人 review 时很烦）。
...
- 诊断类：`leafcli doctor` → 看 `notes.md` → 搜 autofix skill
