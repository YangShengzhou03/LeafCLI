import { ArgumentError, CommandExecutionError, EmptyResultError, selectorError } from '@yangshengzhou/leafcli/errors';
  ...
  return `
    (function(kinds, click, limit, maxChars, markCandidates) {
      ...
      for (const candidate of candidates) {
        ...
        if (markCandidates) {
          const marker = 'leafcli-approval-' + rows.length + '-' + Math.random().toString(36).slice(2);
          candidate.button.setAttribute('data-leafcli-approval-id', marker);
          selector = '[data-leafcli-approval-id="' + marker + '"]';
        }
        const row = {
