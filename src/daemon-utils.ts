...
export const PROFILE_DISCONNECTED_HINT =
  'Open that Chrome profile and make sure the leafcli extension is enabled, or choose another profile with leafcli profile use <name>.';
...
export type ProfileRouteInput = {
  /** Hard requirement (--profile / leafcli_PROFILE) — never falls back. */
 ...
 * connected profile, leafcli uses it automatically" true even when a persisted
...
export function resolveProfileRoute(input: ProfileRouteInput): ProfileRouteResult {
  ...
  if (connected.length > 1) {
    return {
      ...
      errorHint: preferred
        ? 'Run leafcli profile list, then update the stale default with leafcli profile use <name> or pass --profile <name>.'
        : 'Run leafcli profile list, then use leafcli --profile <name> ... or leafcli profile use <name>.',
    };
  }
  ...
  return {
    ...
    error: 'Extension not connected. Please install the leafcli Browser Bridge extension.',
  };
}
