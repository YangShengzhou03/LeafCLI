 ...
 * from ~/.leafcli/apps.yaml (additive only, does not override builtins).
 */
...
export interface ElectronAppEntry {
  ...
  port: number;
  /** Process name for detection via Windows tasklist */
  processName: string;
...
function ensureLoaded(): Record<string, ElectronAppEntry> {
  ...
  try {
    const yamlPath = path.join(os.homedir(), '.leafcli', 'apps.yaml');
