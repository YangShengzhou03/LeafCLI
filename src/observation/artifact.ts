...
function baseleafcliDir(): string {
  return process.env.leafcli_CONFIG_DIR || path.join(os.homedir(), '.leafcli');
}
...
export function getTraceDirectory(contextId: string | undefined, traceId: string, baseDir: string = baseleafcliDir()): string {
...
function renderSummary(
  ...
  const lines = [
    '---',
    'schemaVersion: 1',
    `leafcliVersion: ${yamlScalar(PKG_VERSION)}`,
    ...
    '---',
    '',
    '# leafcli Trace Summary',
    '',
    '## How To Use',
    '',
    '- Start with this summary, then inspect `trace.jsonl` only when the evidence below is insufficient.',
    '- For adapter repair policy and retry limits, use the `leafcli-autofix` skill.',
