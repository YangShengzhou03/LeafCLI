...
async function gotoInstagramHome(page, forceReload = false) {
    if (forceReload) {
        await page.goto(`${INSTAGRAM_HOME_URL}?__leafcli_reset=${Date.now()}`);
...
async function findUploadSelectors(page, mediaItems) {
    ...
    ((includesVideo) => {
      ...
      document.querySelectorAll('[data-leafcli-ig-upload-index]').forEach((el) => el.removeAttribute('data-leafcli-ig-upload-index'));
      const selectors = ordered.map((input, index) => {
        input.setAttribute('data-leafcli-ig-upload-index', String(index));
        return '[data-leafcli-ig-upload-index="' + index + '"]';
      });
...
function extractSelectorIndex(selector) {
    const match = selector.match(/data-leafcli-ig-upload-index="(\d+)"/);
    if (!match)
        return null;
...
async function injectImageViaBrowser(page, imagePaths, selector) {
    ...
    const chunkKey = `__leafcliInstagramUpload_${Date.now()}_${Math.random().toString(36).slice(2)}`;
...
cli({
    ...
    func: async (page, kwargs) => {
        ...
        const protocolCaptureEnabled = process.env.leafcli_INSTAGRAM_CAPTURE === '1';
