...
export async function resolveInstagramPrivatePublishConfig(page) {
    ...
    for (let attempt = 0; attempt < INSTAGRAM_PRIVATE_CONFIG_RETRY_BUDGET; attempt += 1) {
        try {
            ...
            await page.goto(`${INSTAGRAM_HOME_URL}?__leafcli_private_probe=${Date.now()}`);
...
function buildPrivateNormalizedImagePath(filePath) {
    ...
    return path.join(os.tmpdir(), `leafcli-instagram-private-${parsed.name}-${crypto.randomUUID()}${parsed.ext || '.png'}`);
}
...
function runSwiftJsonScript(script, args, stage) {
    const scriptPath = path.join(os.tmpdir(), `leafcli-instagram-${crypto.randomUUID()}.swift`);
...
function buildPrivateVideoCoverPath(filePath) {
    const parsed = path.parse(filePath);
    return path.join(os.tmpdir(), `leafcli-instagram-private-video-cover-${parsed.name}-${crypto.randomUUID()}.jpg`);
}
function buildPrivateStoryVideoPath(filePath) {
    const parsed = path.parse(filePath);
    return path.join(os.tmpdir(), `leafcli-instagram-story-video-${parsed.name}-${crypto.randomUUID()}${parsed.ext || '.mp4'}`);
}
