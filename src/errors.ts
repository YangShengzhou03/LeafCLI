/**
 * Unified error types for leafcli.
 *
 ...
 * ## Exit codes
 *
 * leafcli follows Unix conventions (sysexits.h) for process exit codes:
 *
 *   0   Success
...
const TRACE_RECEIPT_SYMBOL = Symbol.for('leafcli.traceReceipt');
...
export class TimeoutError extends CliError {
  constructor(label: string, seconds: number, hint?: string) {
    super(
      ...
      hint ?? 'Try again, or increase timeout with --timeout <seconds> (or leafcli_BROWSER_COMMAND_TIMEOUT for the global default)',
      EXIT_CODES.TEMPFAIL,
    );
  }
}
