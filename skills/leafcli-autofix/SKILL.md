---
name: leafcli-autofix
description: Automatically fix broken LeafCLI adapters when commands fail. Load this skill when an leafcli command fails — it guides you through collecting a trace artifact, patching the adapter, retrying, and filing an upstream GitHub issue after a verified fix. Works with any AI agent.
allowed-tools: Bash(leafcli:*), Bash(gh:*), Read, Edit, Write
---

# LeafCLI AutoFix — Automatic Adapter Self-Repair

When an `leafcli` command fails because a website changed its DOM, API, or response schema, **automatically diagnose, fix the adapter, and retry** — don't just report the error.
...
- **`BROWSER_CONNECT`** (exit code 69) — **STOP.** Do not modify code. Tell the user to run `leafcli doctor`.
- **CAPTCHA / rate limiting** — **STOP.** Not an adapter issue.

**Scope constraint:**
- **Only modify the file at `adapterSourcePath` in the trace `summary.md` front matter** — this is the authoritative adapter location (may be `clis/<site>/` in repo or `~/.leafcli/clis/<site>/` for npm installs)
...
```bash
leafcli doctor    # Verify extension + daemon connectivity
```

## When to Use This Skill

Use when `leafcli <site> <command>` fails with repairable errors:
...
- **Retry with an alternative query or entry point.** If `leafcli xiaohongshu search "X"` returns 0 but `leafcli xiaohongshu search "X 攻略"` returns 20, the adapter is fine — the platform was shaping results for the first query.
- **Spot-check in a normal Chrome tab.** If the data is visible in the user's own browser but the adapter comes back empty, the issue is usually authentication state, rate limiting, or a soft block — not a code bug. The fix is `leafcli doctor` / re-login, not editing source.
...
```bash
leafcli <site> <command> [args...] --trace retain-on-failure 2>trace-error.yaml
```
...
trace:
  ...
  leafcliVersion: "..."
  traceId: "..."
  dir: "/path/to/.leafcli/profiles/default/traces/..."
  summaryPath: "/path/to/.leafcli/profiles/default/traces/.../summary.md"
  receiptPath: "/path/to/.leafcli/profiles/default/traces/.../receipt.json"
```
...
Use `leafcli browser` to inspect the live website. **Never use the broken adapter** — it will just fail again.

### DOM changed (SELECTOR errors)

```bash
# Open the page and inspect current DOM
leafcli browser open https://example.com/target-page && leafcli browser state
...
leafcli browser click <N> && leafcli browser network

# Narrow to the request you care about by the fields its body should have
leafcli browser network --filter author,text,likes

# Inspect specific API response (key is the `key` field from the default JSON output)
leafcli browser network --detail <key>
```

## Step 4: Patch the Adapter

Read the adapter source file at `adapterSourcePath` from the trace summary front matter and make targeted fixes. This path is authoritative — it may be in the repo (`clis/`) or user-local (`~/.leafcli/clis/`).
...
```markdown
## Summary
LeafCLI autofix repaired this adapter locally, and the retry passed.

## Adapter
- Site: `<site>`
- Command: `<command>`
- LeafCLI version: `<version from leafcli --version>`
...
_Issue filed by LeafCLI autofix after a verified local repair._
```
...
- **Major redesign** — needs full adapter rewrite via `leafcli-adapter-author` skill
...
```
1. User runs: leafcli zhihu hot
   → Fails: SELECTOR "Could not find element: .HotList-item"

2. AI runs: leafcli zhihu hot --trace retain-on-failure 2>trace-error.yaml
...
4. AI explores: leafcli browser open https://www.zhihu.com/hot && leafcli browser state
...
6. AI verifies: leafcli zhihu hot
