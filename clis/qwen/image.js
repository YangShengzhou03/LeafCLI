...
import { ArgumentError, CommandExecutionError, EmptyResultError, TimeoutError } from '@yangshengzhou/leafcli/errors';
import {
    QIANWEN_DOMAIN,
    authRequired,
    dismissLoginModal,
...
cli({
    ...
    func: async (page, kwargs) => {
        ...
        if (skipDownload) {
            return [{ Status: '[已生成] generated', File: null, Link: link }];
        }
        ...
        for (let i = 0; i < urls.length; i += 1) {
            ...
            results.push({ Status: '[成功] saved', File: displayPath(filePath), Link: link });
        }
        if (!results.length) {
