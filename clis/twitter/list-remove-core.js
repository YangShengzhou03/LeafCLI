...
export async function listRemoveUser(page, kwargs) {
        ...
        const uiResult = unwrapBrowserResult(await page.evaluate(`(async () => {
            ...
            try {
                if (!window.__leafcliListMutations) {
                    window.__leafcliListMutations = [];
                    ...
                    window.fetch = async function(...args) {
                        ...
                        try { resp = await origFetch(...args); } catch (err) {
                            if (/ListAddMember|ListRemoveMember|lists\\/members\\/(create|destroy)/.test(url)) {
                                window.__leafcliListMutations.push({ url, method, status: 0, error: String(err), ts: Date.now() });
                            }
                            throw err;
                        }
                        if (/ListAddMember|ListRemoveMember|lists\\/members\\/(create|destroy)/.test(url)) {
                            window.__leafcliListMutations.push({ url, method, status: resp.status, ts: Date.now() });
                        }
                        return resp;
                    };
                }
                window.__leafcliListMutations.length = 0;
                ...
                return {
                    ...
                    mutationsBefore: window.__leafcliListMutations.length,
                };
            } catch (e) {
