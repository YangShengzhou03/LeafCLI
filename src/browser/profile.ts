...
function profileConfigPath(): string {
  const baseDir = process.env.leafcli_CONFIG_DIR || path.join(os.homedir(), '.leafcli');
...
export type ProfileSelection = {
   ...
   * or leafcli_PROFILE env): route strictly and fail loud if it is offline.
...
export function resolveProfileSelection(profile?: string): ProfileSelection | undefined {
  ...
  const explicit = normalizeContextId(profile) ?? normalizeContextId(process.env.leafcli_PROFILE);
