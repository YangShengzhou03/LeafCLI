...
export interface ExternalCliConfig {
  /** User-facing leafcli subcommand and, by default, the executable name. */
  name: string;
  binary: string;
  /**
...
function getUserRegistryPath(): string {
  ...
  return path.join(home, '.leafcli', 'external-clis.yaml');
}
...
export function getInstallCmd(installConfig?: ExternalCliInstall): string | null {
  ...
  if (installConfig.windows) return installConfig.windows;
 ...
 *
 * Args:
 *   cmd: Raw command string from YAML config (e.g. "npm install -g gh")
 *
 * Returns:
