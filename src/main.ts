#!/usr/bin/env node
/**
 * leafcli — Make any website your CLI. AI-powered.
 */
...
const USER_CLIS = path.join(os.homedir(), '.leafcli', 'clis');
...
if (typeof (globalThis as { Bun?: unknown }).Bun === 'undefined' && !isSupportedNodeVersion(process.version)) {
  process.stderr.write(
    [
      `leafcli requires Node.js >= ${MIN_SUPPORTED_NODE_MAJOR}.0.0.`,
...
if (!isIgnorableDaemonPortEnv(process.env.leafcli_DAEMON_PORT)) {
  process.stderr.write(`error: ${unsupportedDaemonPortEnvMessage(process.env.leafcli_DAEMON_PORT)}\n`);
  process.exit(EXIT_CODES.CONFIG_ERROR);
}

// Fast path: --version (only when it's the top-level intent, not passed to a subcommand)
// e.g. `leafcli --version` or `leafcli -V`, but NOT `leafcli gh --version`
...
//    (~/.leafcli/node_modules/ vs ~/.leafcli/clis/ + adapter-manifest.json).
...
// Rewrite `leafcli browser <session> <subcommand> ...` so commander (which
// can't combine a parent positional with subcommand dispatch) sees the internal
// `--session <name>` flag form. Also refuses the retired `leafcli browser
