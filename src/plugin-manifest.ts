/**
 * Plugin manifest: reads and validates leafcli-plugin.json files.
 *
 * Supports two modes:
...
export interface SubPluginEntry {
  ...
  version?: string;
  description?: string;
  /** Semver range for leafcli compatibility (overrides top-level). */
  leafcli?: string;
...
export interface PluginManifest {
  ...
  version?: string;
  /** Semver range for leafcli compatibility, e.g. ">=1.0.0". */
  leafcli?: string;
...
export const MANIFEST_FILENAME = 'leafcli-plugin.json';

// ── Read / Validate ─────────────────────────────────────────────────────────

/**
 * Read and parse leafcli-plugin.json from a directory.
...
/**
 * Check if the current leafcli version satisfies a semver range string.
 *
