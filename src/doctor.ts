...
export async function runBrowserDoctor(opts: DoctorOptions = {}): Promise<DoctorReport> {
  ...
  if (daemonFlaky) {
    ...
    issues.push('Daemon is not running. It should start automatically when you run an leafcli browser command.');
  }
  ...
  if (extensionFlaky) {
    ...
    if (health.state === 'profile-required') {
      issues.push(
        ...
        '  Run leafcli profile list, then leafcli profile use <name>, or pass --profile <name>.',
      );
    } else if (health.state === 'profile-disconnected') {
      issues.push(
        `Selected browser profile is not connected: ${health.status?.contextId ?? 'unknown'}.\n` +
        '  Open that Chrome profile and make sure the leafcli extension is enabled.',
      );
    } else {
      issues.push(
  ...
  if (staleDefault && profiles?.length && !profiles.some((p) => p.contextId === staleDefault)) {
    ...
    issues.push(
      ...
      `  ${fallbackNote}\n` +
      '  Refresh it with: leafcli profile list, then leafcli profile use <name>.',
    );
  }
...
export function renderBrowserDoctorReport(report: DoctorReport): string {
  const lines = [`leafcli v${report.cliVersion ?? 'unknown'} doctor` + ` (${getRuntimeLabel()})`, ''];
