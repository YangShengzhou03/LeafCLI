...
/**
 * Sparse adapter sync: keeps ~/.leafcli/clis/ clean by removing stale overrides.
 *
 ...
 *
 * ~/.leafcli/clis/ is a sparse override layer, not a full copy.
 * Only eject-ed or user-modified sites appear here.
 *
 * Only runs on global install (npm install -g) or explicit leafcli_FETCH=1.
...
const leafcli_DIR = join(homedir(), '.leafcli');
const USER_CLIS_DIR = join(leafcli_DIR, 'clis');
const MANIFEST_PATH = join(leafcli_DIR, 'adapter-manifest.json');
const PACKAGE_ROOT = resolve(import.meta.dirname, '..');
const BUILTIN_CLIS = join(PACKAGE_ROOT, 'clis');

function log(msg) {
  console.log(`[leafcli] ${msg}`);
}
...
export function fetchAdapters() {
  ...
  // Skip if already installed at the same version (unless forced via leafcli_FETCH=1)
  const isForced = process.env.leafcli_FETCH === '1';
  ...
  //    Only eject-ed sites live in ~/.leafcli/clis/.
  let cleared = 0;
  ...
  // 4. Clean up legacy compat shim files from ~/.leafcli/
  // These were created by an older approach that placed re-export shims directly
  // in ~/.leafcli/ (e.g., registry.js, errors.js, browser/). The current approach
  // uses a node_modules/@yangshengzhou/leafcli symlink instead.
  ...
  let tmpCleaned = 0;
  try {
    for (const entry of readdirSync(leafcli_DIR)) {
      if (entry.startsWith('.plugins.lock.json.tmp-')) {
        try {
          unlinkSync(join(leafcli_DIR, entry));
          tmpCleaned++;
        } catch { /* ignore */ }
      }
    }
...
function main() {
  ...
  const isExplicit = process.env.leafcli_FETCH === '1';
  const isFirstRun = process.env._leafcli_FIRST_RUN === '1';
