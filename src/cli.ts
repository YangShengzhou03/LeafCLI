...
import { listleafcliSkills, readleafcliSkill } from './skills.js';
 ...
 * the JS interceptor's `window.__leafcli_net`) into a consistent shape.
...
async function captureNetworkItems(page: import('./types.js').IPage): Promise<BrowserNetworkItem[]> {
  ...
  const raw = await page.evaluate(`(function(){ var out = window.__leafcli_net || []; window.__leafcli_net = []; return JSON.stringify(out); })()`) as string;
  try {
    const parsed = JSON.parse(raw) as BrowserNetworkItem[];
    return parsed.map((item) => ({ ...item, timestamp: timestampFromRaw(item.timestamp) }));
  } catch {
    if (process.env.leafcli_VERBOSE) log.warn(`[network] Failed to parse interceptor buffer: ${typeof raw === 'string' ? raw.slice(0, 200) : String(raw)}`);
    return [];
  }
}
 ...
 * ~/.leafcli/sites/<site>/. Agents have a strong tendency to forget to write
...
const SITEMAP_HINT =
  'Site sitemap available. For navigation context, use the leafcli-browser-sitemap skill; treat browser state as truth if it disagrees.';
...
function sitemapPathsForSite(site: string, opts: Required<Pick<SitemapAvailabilityOptions, 'homeDir' | 'packageRoot' | 'fileExists'>>): { local?: string; global?: string } {
  ...
  if (!safeSite) return {};
  const localBase = path.join(opts.homeDir, '.leafcli', 'sites', safeSite);
  return {
...
export function checkSiteMemory(site: string): SiteMemoryReport {
  const siteDir = path.join(os.homedir(), '.leafcli', 'sites', site);
...
function getBrowserCacheDir(): string {
  return process.env.leafcli_CACHE_DIR || path.join(os.homedir(), '.leafcli', 'cache');
}
...
async function resolveBrowserTargetInSession(
  ...
  try {
    ...
    throw new Error(
      ...
      'The Browser Bridge session may have restarted; re-run "leafcli browser tab list" and choose a current target.',
      { cause: err },
    );
  }
...
async function getBrowserPage(
  ...
  const envTimeout = process.env.leafcli_BROWSER_IDLE_TIMEOUT;
...
function getBrowserWindowMode(command: Command | undefined, defaultMode: BrowserWindowMode): BrowserWindowMode {
  ...
  const envRaw = process.env.leafcli_WINDOW;
  if (envRaw !== undefined && envRaw !== '') {
    if (envRaw === 'foreground' || envRaw === 'background') return envRaw;
    throw new Error(`leafcli_WINDOW must be one of: foreground, background. Received: "${envRaw}"`);
  }
  return defaultMode;
}
...
function getBrowserSession(command?: Command): string {
  // The CLI surface is `leafcli browser <session> <subcommand>`. main.ts rewrites
  ...
  throw new Error('<session> is a required positional argument: leafcli browser <session> <command>');
}
...
function applyVerbose(opts: { verbose?: boolean }): void {
  if (opts.verbose) process.env.leafcli_VERBOSE = '1';
}
...
export function createProgram(BUILTIN_CLIS: string, USER_CLIS: string): Command {
  ...
  program
    ...
    .action((opts) => {
      ...
      console.log();
      console.log('  leafcli' + ' — available commands');
      console.log();
  ...
  const skillsCmd = program
    .command('skills')
    .description('Read bundled leafcli skills');

  skillsCmd
    .command('list')
    .description('List bundled leafcli-* skills')
    .option('-f, --format <fmt>', 'Output format: table, json, yaml, md, csv', 'table')
    .action((opts) => {
      const rows = listleafcliSkills();
      renderOutput(rows, {
        fmt: opts.format,
        fmtExplicit: !!opts.format,
        columns: ['name', 'description', 'version', 'path'],
        title: 'leafcli/skills/list',
        source: 'leafcli skills list',
      });
    });

  skillsCmd
    .command('read')
    .description("Print an leafcli-* skill's SKILL.md or reference file")
    .argument('<skill>', 'Skill name, or skill/path like leafcli-browser/references/foo.md')
    ...
    .action((skill: string, skillPath: string | undefined, opts) => {
      let result: ReturnType<typeof readleafcliSkill>;
      try {
        result = readleafcliSkill(skill, skillPath ?? '');
      } catch (err) {
...
Examples:
  $ leafcli browser work open https://x.com
  $ leafcli browser work open https://x.com --window background
  $ leafcli browser work click 12
  $ leafcli browser work state
  $ leafcli browser work bind
  $ leafcli browser work unbind
`);
  ...
  function emitJavaScriptDialogError(message: string): void {
    console.log(JSON.stringify({
      error: {
        ...
        message,
        hint: 'Handle the modal first: leafcli browser dialog accept (or dismiss). Use --text for prompt dialogs.',
      },
    }, null, 2));
  }
      ...
      if ((!opts.css || typeof opts.css !== 'string') && !locator) {
        console.log(JSON.stringify({
          error: {
            ...
            hint: 'Examples: leafcli browser find --css ".btn.primary"; leafcli browser find --role button --name Save',
          },
        }, null, 2));
        process.exitCode = EXIT_CODES.USAGE_ERROR;
        return;
  ...
  addBrowserTabOption(
    ...
    .action(browserAction(async (page, opts) => {
      ...
      if (max > 0 && html.length > max) {
        console.log(`<!-- leafcli: truncated ${max} of ${html.length} chars; re-run without --max (or --max 0) for full -->\n${html.slice(0, max)}`);
        return;
      }
      console.log(html);
    }));
    ...
    if (inputs.length === 0) {
      return {
        error: {
          ...
          hint: 'Example: leafcli browser upload "input[type=file]" ./receipt.pdf',
        },
      };
    }
    const files: string[] = [];
  ...
  // Captures are persisted per browser session under ~/.leafcli/cache/browser-network/.
  ...
  browser.command('init')
    ...
    .description('Generate adapter scaffold in ~/.leafcli/clis/')
    .action(async (name: string) => {
      try {
        ...
        const dir = path.join(os.homedir(), '.leafcli', 'clis', site);
        ...
        const template = `import { cli, Strategy } from '@yangshengzhou/leafcli/registry';
...
cli({
  ...
  example: 'leafcli ${site} ${command} -f yaml',
  domain: '${domain}',
...
`;
        ...
        console.log('First time on this site? Run: leafcli browser analyze <url>');
        console.log(`Edit the file to implement your adapter, then run: leafcli browser verify ${name}`);
      } catch (err) {
  ...
  browser.command('verify')
    .argument('<name>', 'Adapter name in site/command format (e.g. hn/top)')
    .option('--write-fixture', 'Write a starter fixture to ~/.leafcli/sites/<site>/verify/<command>.json if none exists')
    ...
    .option('--strict-memory', 'Fail (not just warn) when ~/.leafcli/sites/<site>/endpoints.json or notes.md is missing')
    ...
    .description('Execute an adapter and validate output; uses fixture at ~/.leafcli/sites/<site>/verify/<cmd>.json when present')
    .action(async (name: string, opts: { fixture?: boolean; writeFixture?: boolean; updateFixture?: boolean; strictMemory?: boolean; seedArgs?: string; trace?: string } = {}) => {
      try {
        ...
        const filePath = path.join(os.homedir(), '.leafcli', 'clis', site, `${command}.js`);
        if (!fs.existsSync(filePath)) {
          console.error(`Adapter not found: ${filePath}`);
          console.error(`Run "leafcli browser init ${name}" to create it.`);
          process.exitCode = EXIT_CODES.GENERIC_ERROR;
          return;
        }

        console.log(`[验证] Verifying ${name}...\n`);
        ...
        try {
          ...
          console.log(`  Executing: leafcli ${site} ${command} ${argDisplay}\n`);
          ...
          console.log(`\n  [失败] Adapter failed. Fix the code and try again.`);
          process.exitCode = EXIT_CODES.GENERIC_ERROR;
          return;
        }
        ...
        try {
          ...
          console.log(rawJson);
          console.log('\n  [失败] Could not parse adapter output as JSON. Is `--format json` broken?');
          process.exitCode = EXIT_CODES.GENERIC_ERROR;
          return;
        }
        ...
        if (shapeFailures.length > 0) {
          console.log(`\n  [失败] Adapter output violates row shape conventions:`);
        ...
        if (!fixture) {
          console.log(`\n  [成功] Adapter runs. (No fixture at ${fixturePath(site, command)} — consider --write-fixture to seed one.)`);
        ...
        if (failures.length === 0) {
          console.log(`\n  [成功] Adapter matches fixture (${fixturePath(site, command)}).`);
          const memoryReport = checkSiteMemory(site);
          printSiteMemoryReport(memoryReport, opts.strictMemory);
          if (!memoryReport.ok && opts.strictMemory) {
            process.exitCode = EXIT_CODES.GENERIC_ERROR;
        ...
        console.log(`\n  [失败] Adapter output does not match fixture:`);
  ...
  program
    .command('doctor')
    .description('Diagnose leafcli browser bridge connectivity')
  ...
  const pluginCmd = program.command('plugin').description('Manage leafcli plugins');
      ...
      try {
        ...
        if (Array.isArray(result)) {
          if (result.length === 0) {
            ...
            console.log(`[成功] Installed ${result.length} plugin(s) from monorepo: ${result.join(', ')}`);
          }
        } else {
          console.log(`[成功] Plugin "${result}" installed successfully. Commands are ready to use.`);
        }
      } catch (err) {
        console.error(`Error: ${getErrorMessage(err)}`);
        process.exitCode = EXIT_CODES.GENERIC_ERROR;
  ...
  pluginCmd
    ...
    .action(async (name: string) => {
      ...
      try {
        ...
        console.log(`[成功] Plugin "${name}" uninstalled.`);
      } catch (err) {
        console.error(`Error: ${getErrorMessage(err)}`);
        process.exitCode = EXIT_CODES.GENERIC_ERROR;
      }
    ...
    .action(async (name: string | undefined, opts: { all?: boolean }) => {
      ...
      if (opts.all) {
        ...
        for (const result of results) {
          ...
          hasErrors = true;
          console.log(`  [失败] ${result.name} — ${String(result.error)}`);
        }
        ...
        if (hasErrors) {
          ...
          console.log('[成功] All plugins updated successfully.');
        }
        return;
      }
      ...
      try {
        updatePlugin(name!);
        await discoverPlugins();
        console.log(`[成功] Plugin "${name}" updated successfully.`);
      } catch (err) {
        console.error(`Error: ${getErrorMessage(err)}`);
        process.exitCode = EXIT_CODES.GENERIC_ERROR;
      }
  ...
  pluginCmd
    ...
    .action(async (opts) => {
      ...
      if (plugins.length === 0) {
        console.log('  No plugins installed.');
        console.log('  Install one with: leafcli plugin install github:user/repo');
        return;
      }
      if (opts.format === 'json') {
        renderOutput(plugins, {
          ...
          title: 'leafcli/plugins',
          source: 'leafcli plugin list',
        });
        return;
      }
      console.log();
  ...
  pluginCmd
    ...
    .action(async (name: string, opts: { dir?: string; description?: string }) => {
      ...
      try {
        ...
        console.log(`[成功] Plugin scaffold created at ${result.dir}`);
        console.log();
        ...
        console.log(`    leafcli plugin install file://${result.dir}`);
        console.log(`    leafcli ${name} hello`);
      } catch (err) {
        console.error(`Error: ${getErrorMessage(err)}`);
        process.exitCode = EXIT_CODES.GENERIC_ERROR;
      }
  ...
  adapterCmd
    ...
    .action(async () => {
      const os = await import('node:os');
      const userClisDir = path.join(os.homedir(), '.leafcli', 'clis');
      ...
      try {
        ...
        console.log(`Local overrides in ~/.leafcli/clis/ (${userSites.length} sites):\n`);
  ...
  adapterCmd
    .command('eject')
    .description('Copy an official adapter to ~/.leafcli/clis/ for local editing')
    ...
    .action(async (site: string) => {
      ...
      try {
        ...
        console.error(`Site "${site}" already exists in ~/.leafcli/clis/. Use "leafcli adapter reset ${site}" first to restore official version.`);
        process.exitCode = EXIT_CODES.USAGE_ERROR;
        return;
      } catch { /* good, doesn't exist yet */ }

      fs.cpSync(builtinSiteDir, userSiteDir, { recursive: true });
      console.log(`[成功] Ejected "${site}" to ~/.leafcli/clis/${site}/`);
    ...
    .action(async (site: string | undefined, opts: { all?: boolean }) => {
      ...
      if (opts.all) {
        try {
          ...
          console.log(`[成功] Reset ${dirs.length} site(s). All adapters now use official baseline.`);
        } catch {
          console.log('No local sites to reset.');
        }
        return;
      ...
      console.log(isOfficial
        ? `[成功] Reset "${site}". Now using official baseline.`
        : `[成功] Removed custom site "${site}".`);
    });
  ...
  profileCmd
    ...
    .action(async () => {
      ...
      if (!status) {
        console.log('Daemon is not running. Run leafcli doctor after opening Chrome.');
        return;
      }
      if (isDaemonStale(status, PKG_VERSION) || !Array.isArray(status.profiles)) {
        ...
        console.log('Run: leafcli daemon restart');
        return;
      }
      if (profiles.length === 0) {
        console.log('No Browser Bridge profiles connected.');
        console.log('Open a Chrome profile with the leafcli extension installed, then run leafcli profile list again.');
        return;
      }
  ...
  profileCmd
    ...
    .argument('<contextId>', 'Profile contextId from leafcli profile list')
  ...
  // ── Built-in: daemon ──────────────────────────────────────────────────────
  const daemonCmd = program.command('daemon').description('Manage the leafcli daemon');
  // Snapshot before applyRootSubcommandSummaries() rewrites .description() to a child-name listing.
  ...
  taskWatcherCmd
    .command('start')
    .description('Start the task watcher daemon')
    .action(async () => { await taskWatcherStart(); });

  taskWatcherCmd
    .command('stop')
    .description('Stop the task watcher daemon')
    .action(async () => { await taskWatcherStop(); });

  taskWatcherCmd
    .command('status')
    .description('Show task watcher status')
    .action(async () => { await taskWatcherStatus(); });

  const configCmd = taskWatcherCmd
    .command('config')
    .description('Configure task watchers');

  configCmd
    .command('add')
    .description('Add a task page to monitor')
    .argument('<url>', 'URL of the task page (e.g., complaint.example.com)')
    .argument('<template>', 'Trigger template (e.g., "发现新的投诉工单: {{title}}，请处理。工单编号: {{id}}")')
    .option('--interval <seconds>', 'Check interval in seconds', '30')
    .option('--target-ai <ai>', 'Target AI: claude, trae, cursor', 'claude')
    .option('--selector <css>', 'CSS selector for task items')
    .action(async (url: string, template: string, opts) => {
      await taskWatcherConfigAdd(url, template, {
        interval: parseInt(opts.interval, 10),
        targetAI: opts.targetAi,
        taskSelector: opts.selector,
      });
    });

  configCmd
    .command('remove')
    .description('Remove a task watcher')
    .argument('<url>', 'URL of the task page')
    .action(async (url: string) => { await taskWatcherConfigRemove(url); });

  configCmd
    .command('list')
    .description('List configured task watchers')
    .action(async () => { await taskWatcherConfigList(); });
  ...
  externalCmd
    ...
    .action((opts) => {
      ...
      renderOutput(rows, {
        ...
        title: 'leafcli/external/list',
        source: 'leafcli external list',
      });
    });
  ...
  // line is accurate: `Usage: leafcli browser <session> click [target] [options]`
  // instead of `leafcli browser click [target] [options]`. Commander does NOT
  ...
  program.on('command:*', (operands: string[]) => {
    ...
    if (isBinaryInstalled(binary)) {
      console.error(`  Tip: '${binary}' exists on your PATH. Use 'leafcli external register ${binary}' to add it as an external CLI.`);
    }
    program.outputHelp();
    process.exitCode = EXIT_CODES.USAGE_ERROR;
  });
...
export function resolveBrowserVerifyInvocation(opts: {
  ...
  if (!fileExists(sourceEntry)) {
    throw new Error(`Could not find leafcli entrypoint under ${projectRoot}. Expected built entry from package.json or src/main.ts.`);
  }
